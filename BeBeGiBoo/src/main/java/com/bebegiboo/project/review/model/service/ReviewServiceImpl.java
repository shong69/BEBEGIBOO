...
public class ReviewServiceImpl implements ReviewService{
	...
	public Map<String, Object> selectReviewList(int cp) {
		...
		List<Certification> reviewList = mapper.SelectReivewList(rowBounds);
	...
	public Map<String, Object> searchList(Map<String, Object> paramMap, int cp) {
		...
		List<Certification> reviewList = mapper.selectSearchList(paramMap, rowBounds);
		
		//4. 목록 조회 결과 + pagination 객체를 map으로 묶어서 결과로 반환
		Map<String, Object> map = new HashMap<>();
		
		map.put("pagination", pagination);
		map.put("revieList", reviewList);
